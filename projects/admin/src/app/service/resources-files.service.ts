...
import { Record, RecordService, File } from "@rero/ng-core";
...
export class ResourcesFilesService {
  ...
  getParentRecord(type: string, pid: string): Observable<Record> {
    ...
    return this.httpService
      .get(
        `${this.baseUrl}?q=metadata.links.keyword:doc_${pid} AND metadata.owners.keyword:lib_${libPid}`
      )
      .pipe(
        map((result: Record) => {
  ...
  list(
    ...
    return this.httpService
      .get(`${this.baseUrl}/${parentRecord.id}/files`)
      .pipe(
        map((res: any) => {
  ...
  create(
    ...
    // create the bucket
    return this.httpService
      .post(`${this.baseUrl}/${parentRecord.id}/files`, [{ key: fileKey }])
      .pipe(
        switchMap((res: any) =>
          // set the file content
          this.httpService.put(
            `${this.baseUrl}/${parentRecord.id}/files/${fileKey}/content`,
            fileData,
  ...
  update(
    ...
    return this.delete(type, pid, parentRecord, file, true).pipe(
      switchMap((res) =>
        this.create(type, pid, parentRecord, file.key, fileData)
      )
    );
  }
  ...
  delete(
    ...
    return this.httpService
      ...
      .pipe(
        switchMap(() =>
          this.httpService.get(`${this.baseUrl}/${parentRecord.id}/files`)
        ),
        map((res: any) => {
          if (
            keepParent === true ||
            res.entries.some(
              (val) => !["thumbnail", "fulltext"].includes(val?.metadata?.type)
            )
          ) {
            return of(true);
          }
  ...
  updateMetadata(type: string, pid: string, parentRecord: Record, file: File): Observable<any> {
    return this.httpService.put(
      `${this.baseUrl}/${parentRecord.id}/files/${file.key}`,
      file.metadata
    );
  }
}
